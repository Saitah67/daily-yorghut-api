...
def signup():
    if request.method == 'POST':
        ...
        connection = pymysql.connect(host='localhost',user='root',password='',database='Vincent Saitabau_Fungo')
...
def signin():
    ...
    if count == 0:
        return jsonify({"Message":"Log in fail"})
    else:
...
def addProducts():
    if request.method == 'POST':
        ...
        # specifying where the image will be saved in (static folder) - image path
